...
import { useMyProviderProfile, useOrder } from "@/lib/hooks";
import { Avatar } from "@/components/ui/avatar";
import { Screen } from "@/components/ui/screen";

// Mesmo padrão de client/order/[id].tsx, com o rótulo do lado do prestador.
const FLOW: { status: OrderStatus; title: string }[] = [
  { status: "CREATED", title: "Pedido recebido" },
  { status: "ACCEPTED", title: "Você aceitou o pedido" },
  { status: "EN_ROUTE", title: "A caminho do cliente" },
  { status: "IN_PROGRESS", title: "Serviço em execução" },
  { status: "AWAITING_CONFIRMATION", title: "Aguardando confirmação do cliente" },
...
export default function ProviderOrderDetail() {
  ...
  // Detecta a transição pra COMPLETED (cliente confirmou) e manda o prestador
  // direto pra avaliação — só numa mudança de fato, nunca ao abrir a tela com
  // um pedido que já estava concluído.
  const prevStatusRef = useRef<OrderStatus | null>(null);
  useEffect(() => {
    if (!order) return;
    if (prevStatusRef.current === "AWAITING_CONFIRMATION" && order.status === "COMPLETED") {
      router.replace({ pathname: "/provider/rating/[id]", params: { id: order.id } });
    }
  ...
  const canNegotiate = order.status === "CREATED" && !!profile?.allowsNegotiation;
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.clientCard}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {order.client.phone && <Text style={styles.clientMeta}>{order.client.phone}</Text>}
          </View>
          <Pressable
        ...
        {(order.status === "ACCEPTED" || order.status === "EN_ROUTE") && (
          <Pressable
            ...
            onPress={() => router.push({ pathname: "/provider/delivery/[id]", params: { id: order.id } })}
          >
        ...
        {cancelled ? (
          <View style={styles.cancelledCard}>
            ...
            <Text style={styles.cancelledText}>{order.cancelReason ?? "Pedido cancelado."}</Text>
          </View>
        ) : (
        ...
        <View style={styles.costCard}>
          <View style={styles.costRow}>
            <Text style={styles.costLabel}>Valor do serviço (você recebe)</Text>
            <Text style={styles.costValue}>{moneyFromCents(order.priceCents)}</Text>
          </View>
          <View style={styles.costDivider} />
          <View style={styles.costRow}>
            <Text style={styles.costLabelSecondary}>Taxa da plataforma</Text>
            <Text style={styles.costValueSecondary}>{moneyFromCents(order.platformFeeCents)}</Text>
          </View>
          <View style={styles.costRow}>
            <Text style={styles.costLabelSecondary}>Total pago pelo cliente</Text>
            <Text style={styles.costValueSecondary}>{moneyFromCents(order.totalCents)}</Text>
          </View>
        </View>

        {order.status === "AWAITING_CONFIRMATION" && (
          <View style={styles.awaitingCard}>
            <Ionicons name="time-outline" size={18} color="#FF9a52" />
            <Text style={styles.awaitingText}>Aguardando o cliente confirmar a conclusão do serviço.</Text>
          </View>
        )}
        ...
        {canNegotiate && (
          ...
          >
            ...
            <Text style={styles.negotiateLinkText}>Propor outro valor</Text>
          </Pressable>
        )}
        ...
        <Pressable
          onPress={() =>
            router.push({ pathname: "/provider/support-ticket-new", params: { orderId: order.id } })
          }
        >
